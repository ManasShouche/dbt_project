...
-- To land a new topic, copy one of the two tables above and change three
-- things: the table name, RECORD_CONTENT to match the topic's payload, and
-- BASE_LOCATION. RECORD_METADATA must be declared exactly as above -- the
-- connector only adds it automatically on tables it creates itself, not on
-- ones pre-created here, and a table missing it silently loses Kafka
-- lineage (topic/partition/offset) with no error.
